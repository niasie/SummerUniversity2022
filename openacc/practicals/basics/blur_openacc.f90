...
subroutine blur_twice_gpu_naive(nsteps, n, in, out)
  ...
  do istep = 1,nsteps
    ...
    !$acc parallel loop copy(in, buffer)
     do i = 2,n-1
        buffer(i) = blur(i, in, n)
     enddo

     ! TODO: Offload this loop to the GPU
     !$acc parallel loop copy(buffer, out)
     do i = 3,n-2
        out(i) = blur(i, buffer, n)
     enddo

     ! TODO: Offload this loop to the GPU
     !$acc parallel loop copy(in, out)
     do i = 1,n
        in(i) = out(i)
     enddo
  enddo
...
subroutine blur_twice_gpu_nocopies(nsteps, n, in, out)
  ...
  do istep = 1,nsteps
     ...
     ! TODO: Offload this loop to the GPU
     !$acc parallel loop present(buffer, out)
     do i = 3,n-2
        out(i) = blur(i, buffer, n)
     enddo

     ! TODO: Offload this loop to the GPU
     !$acc parallel loop present(in, out)
     do i = 1,n
        in(i) = out(i)
     enddo
  enddo
